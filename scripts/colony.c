...
ref SetFantomGuardsMods(string group, string locator, int nation, string enemygroup)
{
    string emodel,natmod;
    ref Cr;
    ...
    if(nation == PIRATE)
    {
       emodel = "Bandit_" + (rand(6) + 1);
    }
    else
    {
       emodel = "guard_" + natmod + "_" + (rand(7) + 1);
    }
...
ref SetFantomOurAttackForts(string group, string locator)
{
	...
	{
		if (CrOur.chr_ai.group == LAI_GROUP_PLAYER) LAi_SetImmortal(CrOur, 1);
	}
	else
	{
		if (CrOur.chr_ai.group == LAI_GROUP_PLAYER) LAi_SetImmortal(CrOur, 0);
	}
	if (CheckAttribute(pchar, "animal")) CrOur.animal = true;
	else DeleteAttribute(CrOur, "animal");
	// CheatSurfMenu by Cheatsurfer <--
    return CrOur;
}
        ...
        {  // город нации
            ...
            if (j > 0)
			{
				...
				for (i = 0; i < j; i++)
	            {
					...
					sld = SetFantomGuardsMods("goto", "", natEsc, "TOWN_BATTLE_SOLDIERS");
    ...
    {   // не резиденция - ворота или порт
		...
		{   // если есть враги
			...
			{				
				...
				if(rand(5) == 1) sld.MushketType = "mushket3"; // 280313
...
void TWN_FightInTown_OpenNext()
{
	...
	{
		Capture_VillemstadCapture(); // 280313 просто заплатка :(
	}
	// Jason НСО
...
void SetGovenoursToResidence() // Jason: усадка генгуберов и командоров - полностью ручной метод
{
	ref rChar;
	...
	rChar.rank = 25;
	rChar.MultiFighter = 2.5; // мультифайтер
	rChar.MultiShooter = 2.5;
...
int TWN_CityCost(string city)
{
	...
	if (CheckAttribute(rColony, "FortValue"))
	{
		money = (sti(rColony.FortValue) * 250000);
	}
	else
	{
	    money = (25 * 200000);

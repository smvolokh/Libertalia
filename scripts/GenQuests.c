...
void ReasonToFast_MeetPatrolShore(string qName) // генерация патруля
{
	...
	for (i = 0; i < num; i++)
	{
		...
		{					
			...
			{
				if(sti(pchar.questTemp.ReasonToFast.p1) < GetCharacterSPECIAL(pchar, "LUCK") * 2)	
				{
					chr.dialog.currentnode = "First Time";
					pchar.questTemp.ReasonToFast = "MeetPatrolFail";
				}
...
int ReasonToFast_GetVictimShipGoods()
{
	...
	case 17 :
        ...
        break;
	case 18 :
            iGoodIdx = GOOD_TIMBERS;
        break;
	case 18 :
            iGoodIdx = GOOD_EBONY;
        break;
        case 20 :
            iGoodIdx = GOOD_PAPRIKA;
        break;
    }
	return iGoodIdx;
}
...
{
	ref sld = GetCharacter(NPC_GenerateCharacter("CapComission_1" , "mercen_" + (rand(17)+11), "man", "man", 10, sti(pchar.GenQuest.CaptainComission.Nation), -1, true, "quest"));
...
{
	ref sld;
	string CityId, PirateId, PirateName;	
	switch (rand(0))
	{
		case 0:
		CityId = "PuertoPrincipe"; // Пуэрто-Принсипе, Куба 
		PirateId = "Marlow";
		sld = CharacterFromID("Marlow");
		PirateName = "Чёрный Пастор";
		break;
		case 1:
		CityId = "LeFransua"; // Ле-Франсуа на Мартинике
		PirateId = "Barbazon";
		PirateName = "Жак Добряк";
		break;
	}
	pchar.GenQuest.CaptainComission.PiratesCity = CityId;
	pchar.GenQuest.CaptainComission.PirateId = PirateId;
	pchar.GenQuest.CaptainComission.PirateName = PirateName;
}

void CaptainComission_GetFamilyType()
{
	pchar.GenQuest.CaptainComission.FamilyType = drand(3) + 1;
}
...
void CaptainComission_GenerateShips()
{
	...
	else
	{
		...
		switch(rand(2))
		{
			...
			case 1: iShipTypeVictim = SHIP_HEAVYGALEON;	break;
...
void CaptainComission_GenerateCoastalPatrol()
{
	...
	for (i = 1; i <= 3; i++)
    {
		...
		CoastGuard = GetCharacter(NPC_GenerateCharacter("Coastal_Captain0" + i, Model, "man", "man", sti(pchar.rank) + rand(MOD_SKILL_ENEMY_RATE), iNation, 365, true, "officer")); // 3 дня, потом сами пропадут
		...
		CoastGuard.GenQuest.CrewMilitiaMode = true;
		CoastGuard.Ship.Mode = "militia";
		SetCharacterPerk(CoastGuard, "MusketsShoot"); 
...
void CaptainComission_GenerateCaptainInPrison() 
{
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("CapComission_1" , "mercen_"+(rand(14)+13), "man", "man", 10, sti(pchar.GenQuest.CaptainComission.Nation), -1, true, "quest"));
	SetSPECIAL(sld, 10,9,10,5,10,9,10);
	SetRandSelfSkill(sld, 60, 90);
	SetRandShipSkill(sld, 40, 70); 
...
void CaptainComission_GenerateCanoneer(string qName)
{
	...
	if (!LAi_CheckLocatorFree("sit", "sit_front1")) FreeSitLocator(sLoc, "sit_front1");
	FreeSitLocator(sLoc, "sit_base1");
	ChangeCharacterAddressGroup(sld, sLoc, "sit", "sit_base1");
	LAi_SetSitType(sld);
}

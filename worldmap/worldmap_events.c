...
float wdmGetMoral()
{
	...
	for(int i = 0; i < COMPANION_MAX; i++)
	{
		...
		if(cn != -1)
		{
			...
			ch = &characters[cn];
			if(sti(ch.ship.type) != SHIP_NOTUSED)
			{
				if(!CheckAttribute(ch, "ship.dirt_hull"))
				{
					ch.ship.dirt_hull = 0;
					ch.ship.dirt_hull.counter = (1+rand(3));
				}
				
				ch.ship.dirt_hull.counter = (sti(ch.ship.dirt_hull.counter) - 1);

				if(sti(ch.ship.dirt_hull.counter) < 0)
				{
					ch.ship.dirt_hull = (sti(ch.ship.dirt_hull)+1));
					ch.ship.dirt_hull.counter = (1+rand(3));
					Log_TestInfo("Обростание дна +1, следующий прирост через "+ch.ship.dirt_hull.counter+" дней.");
				}
				
				if(sti(ch.ship.dirt_hull) > 100) ch.ship.dirt_hull = 100;
			}
			// <- (обростание дна)
		}
	}
...
void wdmEnterSeaQuest(string _chrId)
{
	...
	//пиратка, квест №7. пиратский корвет Леон
	if (_chrId == "LeonCapitain") 
	{
		AddQuestRecord("Pir_Line_7_Soukins", "15");
	}
	//пиратка, квест №7. тяжелый корвет найден

...
void SetBaseShipData(ref refCharacter)
{
	...
	refCharacter.ship.dirt_hull = rand(30); // Dolphin -> (обростание дна 0-30%)
}
...
bool CheckCharacterItem2(ref _refCharacter,string itemName) // Проверка на предметы с учетом "original_id" (Rasteador)
{
	if( CheckAttribute(_refCharacter,"Items_OriginalID."+itemName) && sti(_refCharacter.Items_OriginalID.(itemName))>0 )	return true;
	else	return false;
}
int GetCharacterItem2(ref _refCharacter,string itemName) // Кол-во предметов с учетом "original_id" (Rasteador)
{
	if(CheckAttribute(_refCharacter,"Items_OriginalID."+itemName))
	{
		return sti(_refCharacter.Items_OriginalID.(itemName));
	}
	return 0;
}
...
void SetEquipedItemToCharacter(ref chref, string groupID, string itemID)
{
	...
	switch(groupID)
	{
	    ...
	    case CIRASS_ITEM_TYPE:
			...
			CheckAndSetOverloadMode(chref);
			if (chref.model == "protocusto" && !CheckAttribute(chref, "CSM.Test")) DoQuestCheckDelay("TalkSelf_Quest", 0.5); // Jason
		break;
		...
		// Dolphin -> (Осечка)
		if(CheckAttribute(arItm, "osechka"))
		{	
		chref.chr_ai.osechka = arItm.osechka;
		} 
		else
		{	
		chref.chr_ai.osechka = 0;
		}
...
{
	if (CheckAttribute(pchar, "CSM.AllArtsCanBeEquipped")) { return true; } // --> CheatSurfMenu by Cheatsurfer
	aref arEquip;
	string sAttr, sItem;
	makearef(arEquip, chref.equip_item);
	int q = GetAttributesNum(arEquip);
...
void UpdateReputation()
{
	...
	pchar.jertva = "true"; // добавил сюда затирание проверок на текущий день Rigel. // думаю работает все не совсем так. есть глюки. но я пока не понимаю из за чего.
...
// метод вызывается ежедневно, уменьшает значение крайних репутаций - эффект забывания. ЭТОТ МЕТОД НЕ РАБОТАЕТ имхо Rigel
void UpdateFame()
{
...
bool IsPCharHaveMushketerModel()
{
	...
	if(	HasSubStr(sModel, "Diego") 	||
 		HasSubStr(sModel, "DonDiego") 	||
		HasSubStr(sModel, "Willy") 	||
 		HasSubStr(sModel, "Pat") 	||
 		HasSubStr(sModel, "Bill") 	||
 		HasSubStr(sModel, "blad")   ||
		HasSubStr(sModel, "Sharle") || 
		HasSubStr(sModel, "Alligator") ||
		HasSubStr(sModel, "BlBorada") ||
		HasSubStr(sModel, "Enrique") ||
		HasSubStr(sModel, "FMQT_mercen") ||   
		HasSubStr(sModel, "Gotfri") || 
		HasSubStr(sModel, "GothicM") ||
		HasSubStr(sModel, "Diego_G") ||  
		HasSubStr(sModel, "Uof") ||
		HasSubStr(sModel, "Ulf") ||
		HasSubStr(sModel, "Resc") ||
		HasSubStr(sModel, "Rene") ||
		HasSubStr(sModel, "Blood5") ||
		HasSubStr(sModel, "Hero1") ||
		HasSubStr(sModel, "BSharp")   ||
		HasSubStr(sModel, "BlackBeard") ||
		HasSubStr(sModel, "Balthrop") ||
		HasSubStr(sModel, "Baltrop") ||
		HasSubStr(sModel, "Beltrop") ||
		HasSubStr(sModel, "Maskog") ||
		HasSubStr(sModel, "Archy") ||
		HasSubStr(sModel, "Alonso")   ||
		HasSubStr(sModel, "BSUnd") ||
		HasSubStr(sModel, "Legran") ||
		HasSubStr(sModel, "Migel_2")   ||
		HasSubStr(sModel, "Cortes")   ||
		HasSubStr(sModel, "Espinosa") ||
		HasSubStr(sModel, "Ezpinoza") ||
		HasSubStr(sModel, "Makcgro") ||   
		HasSubStr(sModel, "Charles") ||   
		HasSubStr(sModel, "Milten") ||
   		HasSubStr(sModel, "Andrey") ||
   		HasSubStr(sModel, "PitHeyn") || 
		HasSubStr(sModel, "DevlinK3") ||
		HasSubStr(sModel, "Addington") ||
		HasSubStr(sModel, "Terminator") ||
		HasSubStr(sModel, "VaterSkel_1") ||
		HasSubStr(sModel, "VaterSkel_2") ||
		HasSubStr(sModel, "VaterSkel_3") ||
		HasSubStr(sModel, "VaterSkel_4") ||
		HasSubStr(sModel, "PotCMob") ||
		HasSubStr(sModel, "PotCMobCap") ||
		HasSubStr(sModel, "skeletcap") ||
		HasSubStr(sModel, "Saymon") ||   
		HasSubStr(sModel, "Gino") ||
		HasSubStr(sModel, "Steys") ||
		HasSubStr(sModel, "Norrington") ||
		HasSubStr(sModel, "Chard") || 
		HasSubStr(sModel, "Isterling") || 
		HasSubStr(sModel, "Yohang") ||
		HasSubStr(sModel, "Pitt_2") ||
		HasSubStr(sModel, "Pitt_3") ||
		HasSubStr(sModel, "Alex_blade") ||
		HasSubStr(sModel, "Hornblauer") ||
		HasSubStr(sModel, "Durand") ||
		HasSubStr(sModel, "Tich") ||
		HasSubStr(sModel, "Cortes") ||
		HasSubStr(sModel, "Ortega") ||
		HasSubStr(sModel, "DeLuck") ||
		HasSubStr(sModel, "Dreik") ||
		HasSubStr(sModel, "Grammon") ||
		HasSubStr(sModel, "luckyjack") ||
		HasSubStr(sModel, "PitBleik") ||
		HasSubStr(sModel, "Kurakin") ||
		HasSubStr(sModel, "Oliver") ||
		HasSubStr(sModel, "Rober_Sur") ||
		HasSubStr(sModel, "Yoris") ||
		HasSubStr(sModel, "Sancho") ||
		HasSubStr(sModel, "Eivory") ||
		HasSubStr(sModel, "Orry") ||
		HasSubStr(sModel, "Connor") ||
		HasSubStr(sModel, "Bannister") ||
		HasSubStr(sModel, "Smit") ||
		HasSubStr(sModel, "Andre_Abel") ||
		HasSubStr(sModel, "BahusCap") ||
		HasSubStr(sModel, "Norrington") ||
		HasSubStr(sModel, "Antu") ||
...
float ChangeIndianRelation(float _val) // Jason: репутация у индейцев
{
   ...
   Log_Info("Отношение индейцев изменилось и равно "+stf(pchar.questTemp.Indian.relation)+" единицам");

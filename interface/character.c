...
void IDoExit(int exitCode)
{
	if (bChangePIRATES)
	{
		...
		if (Pchar.questTemp.GothicMonbar != true)//09/08/07 homo для Монбара даем другое начало
		{				
...
*/
	...
	GameInterface.TABLE_OTHER.tr7.td2.scale = 0.85;
	GameInterface.TABLE_OTHER.tr7.td2.textoffset = "3,0";
	...
	if (xi_refCharacter.id == pchar.id && isMainCharacterPatented())
    {
		...
		GameInterface.TABLE_OTHER.tr7.td2.icon.offset = "60, 1";
		GameInterface.TABLE_OTHER.tr7.td2.icon.width = 24;
		GameInterface.TABLE_OTHER.tr7.td2.icon.height = 24;
    }
    else
    {
...
void IncreaseSkill(int _add)
{
    ...
    else
    {
	    ...
	    { //для начала игры пересчет скилов
		    ...
		    if (!CheckAttribute(pchar, "CSM.SPECIALsimpleValue")) InitStartParam(xi_refCharacter); // <-- CheatSurfMenu by Cheatsurfer
		    SetEnergyToCharacter(xi_refCharacter);
	    }
    }
	SetVariable();

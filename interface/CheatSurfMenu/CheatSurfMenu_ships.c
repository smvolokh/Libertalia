...
string descF9 = "Королевский фрегат 'Гриффондор'";
void CalculateInfoDataF9()
{
	totalInfo = "#9 " + descF9;
	CommonActions();
	SetNewPicture("CSMPICTURE", "INTERFACES\Ships\La_Marianna.tga");
	pchar.Ship.Type = GenerateShipExt(SHIP_LA_MARIANNA, true, pchar);
	...
	iSND = PlaySound("_CheatSurfMenu_\ShipGood.wav");
	Log_info("Королевский фрегат 'Гриффондор' " + sTextA);
	totalInfo += back2totalInfo;
	SetFormatedText("INFO_TEXT", totalInfo);

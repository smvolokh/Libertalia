...
void Fort_Login(int iIslandIndex)
{
	...
	for (int i=0; i<iNumReloads; i++)
	{
		...
		if (CheckAttribute(arLocator,"fort"))
		{
			...
			LayerAddObject("sea_reflection", &Forts[iNumForts], 1); // ugeen - добавим отражение форта в воде
			...
			if (sti(rCharacter.nation) != PIRATE || colonies[iColony].id == "SanAndres")
			{
...
void Fort_CheckAttributes(ref rCharacter)
{
	...
	{ 
		...
		rCharacter.Fort.Cannons.Type.1 = -1;//CANNON_TYPE_CULVERINE_LBS18; 
	}

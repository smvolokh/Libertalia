...
float Ship_MastDamage()
{
	...
	// CSM -->
	if (RealShips[sti(rCharacter.Ship.Type)].CSM.MastNoFall == true || CheckAttribute(rCharacter, "CSM.MastNoFall"))
	{
		fDamage = 0.0;
	}
	// CSM <--
...
float Ship_HullDamage()
{
	...
	if (fDamage >= 1.0)
	{
		...
		if(HasSubStr(HullName, "baller")) // потеряли перо руля
		{
			...
			string sDamagedString = "На судне ''" + rCharacter.Ship.Name + "'' повреждён руль, судно потеряло управление.";
...
void Ship_BortReloadEvent()
{
	...
	if (sti(aCharacter.Tmp.BortsReloaded.Event))
	{
		...
		bool bFront = sti(aCharacter.Tmp.BortsReloaded.cannonf) == true;
		bool bBack = sti(aCharacter.Tmp.BortsReloaded.cannonb) == true;
        ...
        if (bLeft && bRight && bFront && bBack)
        {
			...
			PlaySound("interface\_Gun_All_Ready_"+rand(14)+".wav");
		}
		else
		{
			if (bLeft)  { PlaySound("interface\_GunReadyL"+rand(14)+".wav"); }
			if (bRight) { PlaySound("interface\_GunReadyR"+rand(14)+".wav"); }
			if (bBack)  { PlaySound("interface\_Gun_B_Ready_"+rand(14)+".wav"); }
			if (bFront) { PlaySound("interface\_Gun_F_Ready_"+rand(13)+".wav"); }
...
void Ship_SetSurrendered(ref rCharacter)
{
	...
	rCharacter.Coastal_Captain   = true;
...
bool Ship_CheckSurrendered(ref echr) // Это определение сдающегося корабля. Если хотим что-то изменить в сдаче - меняем тут Rigel
{
	bool bOk = true;
	...
	if (sti(echr.Nation) == PIRATE || CheckAttribute(echr, "DontRansackCaptain")) return false; // квестовые не выбрасывают белый флаг. пираты тоже (им как бы нечего терять, все одно в петлю или на дно)
	
	ref mchr = GetMainCharacter();
	
	if (sti(echr.rank) > (900 - MOD_SKILL_ENEMY_RATE)) bOk = false; 	// странная функция. если ранг нпс высокий он не сдастся. установил порог в 900. пусть все кто ниже сдаться могут :)
	
	int eclass = GetCharacterShipClass(echr);
	int mclass = GetCharacterShipClass(mchr);
	if (eclass == 1) bOk = false; // если враг на линкорчике он не струсит что бы ни случилось. ну оставим. вроде вполне реально
	...
	float fCrewRate = 7.0;  // это типа рейтинг крутости в зависимости от профессии НПС - у торговца был почему-то выше чем у вояки. может я что-то не понимаю? изменил. Rigel
	if (CheckAttribute(echr, "Ship.Mode")) 
	{
		switch(echr.Ship.Mode) // с помощью этого параметра удобно корректировать шанс выхода из боя кораблей противника
		{
			case "Trade":
				fCrewRate = 5.0;
			break;
	...
	float mcrew = stf(GetOptCrewQuantity(mchr)) / 100; // 0...10 комп видит мою команду всегда как оптимальную и ведет учет исходя из этого Rigel (не забыть убрать у подзорных труб возможность видеть число людей у врага)
	float ecrew = stf(GetCrewQuantity(echr)) / 100; // 0...10 комп видит свою команду реальной и ведет учет исходя из этого Rigel
	
	float msila = (100 / mclass); // 1.6 ... 10 а вот и новый параметр на который будут ориентироваться НПС в вопросе драпать или нет.
	float esila = (100 / eclass); // 1.6 ... 10 сделал значение только класса корабля, так как это гораздо важнее чем число народа имхо! Rigel. но комп будет сопоставлять это со своей командой.
	// зачем 10 делить - чтобы проще было считать. теперь 1 класс круче чем 6.
	
	// float fRep    = abs(REPUTATION_NEUTRAL - sti(mchr.reputation.nobility)) / 50.0; // приведение к 0..1  /// MIN 1, MAX 90, REPUTATION_NEUTRAL      45
	// float emorale = stf(echr.ship.crew.morale) / MORALE_MAX; // 0..1 это рандом // rigel - я это не использую так как шип енеми не имеет морали ((
	// float mmorale = stf(mchr.ship.crew.morale) / MORALE_MAX; // 0..1
	// float mskill  = (GetSummonSkillFromNameToOld(mchr, "Leadership") + GetSummonSkillFromNameToOld(mchr, "Grappling")) / 20.0;  // 0..10 - не использую так как враг этого не видит
	float eskill  = (GetCharacterSkillToOld(echr, "Leadership") + GetCharacterSkillToOld(echr, "Defence")) / 20.0;    // 0..10
	float mkorp   = (GetHullPercent(mchr) / 100); // комп оценивает процент своего и нашего здоровья и парусов Rigel. в идеале для корпусов нужны не проценты а натуральные числа.
	float ekorp   =	(GetHullPercent(echr) / 100);
	float mpar    = (GetSailPercent(mchr) / 100);
	float epar    = (GetSailPercent(echr) / 100);
	float mspeed  = (stf(RealShips[sti(Pchar.Ship.Type)].SpeedRate));
	
	int mShip = GetCompanionQuantity(mchr); //число моих кораблей в эскадре равно моим компаньонам
    int eShip;
    string sGroupID = Ship_GetGroupID(echr);
    if (sGroupID != "") eShip = Group_GetLiveCharactersNum(sGroupID); //число его кораблей в эскадре равно sGroupID
    else				eShip = 1; //если группы нет, комп считает свою группу равной одному. Rigel
	
    msila = msila + (((mShip) - 1) * 30); // 1.7 ... 10.2 * 3 и того 5.1 ... 30.6          было так - (1.0 + (mShip-1) / 5.0) Rigel
    esila = esila + (((eShip) - 1) * 30); // 1.7 ... 10.2 * 3 и того 5.1 ... 30.6          если выходят 1 на 1 то враг не сваливал. как не сваливал если бы если б на 2 ссанцые шлюпки вышел 1 мановар. слишком силная привязка к числу эскадры
	
	msila = (msila + (mcrew / 100)); //  делим число команды на 100 чтобы цифры были поменьше. это не так важно.
	esila = (esila + (ecrew / 100)); 
	
	// отвязал вообще привязку к морали команд - у компа она не адекватная имхо. Rigel. знать бы как выставить компу мораль не рандомную а адекватную было б норм.
	msila = (msila + 10); //комп не знает наших умений и считает их как чуть ниже средних (4), но умножает их на нащу репутацию
	esila = (esila + (eskill / 5)); //комп свои скилы знает, но его репутация не играет роли. он делит все на 10


	//проверяем силу корпуса
	msila = msila - (100 / GetHullPercent(echr)); //комп смотрит на процент здоровья корпуса гг и прикидывает насколько он опасен. 10 - это уменьшение значимости этого показателя на общее решение драпать. Rigel
	esila = esila - (100 / GetHullPercent(echr)); //комп смотрит на свой собственный корпус...
	//напоследок комп пытается сравнить фактическую возможную скорость ГГ (с учетом наших проблем с парусами) и то насколько повреждены паруса НПС. свою скорость возможную он считает равной 12.
	// if (mShip >= eShip) (msila = msila + (mspeed - (epar / 100 * 12)) / 15);
	//совсем напоследок проверяем насколько у врага железные яйца. вояки почти не сдаются, торговцы бегут // нафиг это
	esila = esila + fCrewRate
	
    if (msila > esila) return true; // сдался
...
// Пират Либерталии ->  работает, фишка интересная, но робит ооочень криво - 16.11.19
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
			        ...
			        if (sti(rCharacter.nation) != PIRATE || bBettaTestMode)//boal 030804 нужно чтоб при тесте флаг с пиратами мирил
			        {
						...
						if (rCharacter.id == "ContraCureerCap")
						{
							if (Ship_GetDistance2D(GetMainCharacter(), rCharacter) < 150)
							{
								...
								ref rCharacter1 = characterFromId("ContraCureerCap");	
								rCharacter1.nation = PIRATE;
								Ship_FlagRefresh(rCharacter1);
...
void Ship_CheckFlagEnemy(ref rCharacter)
{
    ...
    {  // смотрим флаг у ГГ и узнаем в нем...
		...
		if(CheckCharacterPerk(mChar, "Trustworthy")) fPerkInfl = 1000000.0;
		if ((rand(100) + rand(20) + rand(rep)) - GetCharacterSPECIALSimple(pchar, SPECIAL_L)*5 > (fPerkInfl*fSneak * 10 * iClass * (9-iCompan) * isEquippedArtefactUse(mChar, "indian_11", 1.0, 1.15)))
		{
		...
		ChangeCrewExp(mChar, "Sailors", AIShip_isPerksUse(CheckCharacterPerk(mChar, "SeaWolf"), 1.0, 1));
				pchar.questTemp.stels.sea = GetDataDay();
			}
		}
	}
	...
	{
	if (IsCharacterPerkOn(pchar, "Trustworthy")) return; 
...
void Ship_ApplyHullHitpoints(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
	...
	if (!CheckAttribute(pchar, "CSM.NoShipHullPenalty")) // --> CheatSurfMenu
	{
...
void ShipDead(int iDeadCharacterIndex, int iKillStatus, int iKillerCharacterIndex)
{
	...
	if (iDeadCharacterIndex != nMainCharacterIndex)
	{
		...
		Ship_FlagRefresh(rKillerCharacter); //флаг на лету
        }
        else
        {
            if (sKillShipName == "")
...
*/
	...
	if(rDead.id == "GhostCapt" && iKillerCharacterIndex == nMainCharacterIndex && !CheckAttribute(pchar, "GenQuest.GhostShip.LastBattle") && !CheckAttribute(pchar, "GenQuest.GhostShip.Prize"))
	{
		...
		DoQuestCheckDelay("GhostCapt_OfferPrize", 1.5); // Дэви Джонса в студию !
	}
...
void ShipTaken(int iDeadCharacterIndex, int iKillStatus, int iKillerCharacterIndex)
{
	...
	if (iDeadCharacterIndex != GetMainCharacterIndex())
	{
		...
		Ship_FlagRefresh(rDead); //флаг на лету
		Ship_FlagRefresh(rKillerCharacter); //флаг на лету
        }
	}
...
void Ship_HullHitEvent()
{
	...
	if (!CheckAttribute(rOurCharacter, "CSM.NoCannonsBoom")) // --> CheatSurfMenu
	{
...
string GetShipSexWord(string _type, string _man, string _woman)
{
	...
	_type == "Caracca"    	|| 
	_type == "Brigantine" 	||
	_type == "ArmedBrigantine" 	||
	_type == "BattleBrigantine" ||
	_type == "WarXebec"	||
	_type == "LuggerQuest"	||
	_type == "BattleXebec"	||
	_type == "ArmedXebec"	||

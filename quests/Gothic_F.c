...
SetCharacterPerk(sld, "Gunman");
    ...
    {
        sld = GetCharacter(NPC_GenerateCharacter("SpaMush_"+i, "guard_mush_spa_"+(rand(2)+1), "man", "mushketer", Rank, SPAIN, -1, false, "soldier"));
		sld.MusketerDistance = 0;
		sld.MultiFighter = 1.5; // мультифайтер
		sld.MultiShooter = 1.5;
		sld.SuperShooter  = true;
...
//	pchar.questTemp.piratesLine = "Panama_inExitTown"; // Warship FIX 19.07.09 - Затирало флаг по Соукинсу
	...
	for (i=14; i<=17; i++) //ещё испанцы
    {
        if (i==14 || i==16) sTemp = "indsair2";
		else sTemp = "indsar1";
        sld = GetCharacter(NPC_GenerateCharacter("IndEnemy_"+i, "guard_spa_"+(rand(7)+1), "man", "man", 25, SPAIN, 10, true, "soldier"));
		FantomMakeCoolFighter(sld, Rank, 90, 90, "topor_02", "","", 5);
		sld.MultiFighter = 1.5; // мультифайтер
		sld.MultiShooter = 1.5;
		sld.SuperShooter  = true;
...
void No_ChaniSettlement_1(string qName)
{
          ...
          if (pchar.questTemp.GothicLee != "EndOfQuestLine" && pchar.questTemp.GothicLee != "GothicLeeTakeAnna_BadResult" && pchar.questTemp.GothicLee != "GothicLeeTakeAnna_NiceResult" && pchar.questTemp.GothicLee != "AnnaIsLife" && pchar.questTemp.GothicLee != "GothicLeeAnnaHant_GoodWork" && pchar.questTemp.GothicLee != "SeekBible" && pchar.questTemp.GothicLee != "SeekBible_BadResult" && pchar.questTemp.GothicLee != "SeekBible_GoodResult" && pchar.questTemp.GothicLee != "RockBras_toSeekPlace" && pchar.questTemp.GothicLee != "RockBras_RockIsDead" && pchar.questTemp.GothicLee != "RockBras_RockIsSaved" && pchar.questTemp.GothicLee != "RockBras_RockIsAgony" && pchar.questTemp.GothicLee != "ThreeCorsairs_backGovernor" && pchar.questTemp.GothicLee != "GuardPP_GoodWork" && pchar.questTemp.GothicLee != "GuardPP_SoleiRoyalSunk" && pchar.questTemp.GothicLee != "GuardPP_DieHard" && pchar.questTemp.GothicLee != "GuardPP_Late" && pchar.questTemp.GothicLee != "OccupySD_toWeWon" && pchar.questTemp.GothicLee != "QuestLineBreake")
		  {
...
void GothicApostatesAPS_FortOrangetown()
{
	...
	LAi_SetActorType(pchar);
	LAi_ActorGoTolocator(pchar, "patrol", "patrol6", "GothicApostatesAPS_FortOrangetown_1", -1);
}
...
void GothicApostatesAPS_SanAndres_23(string qName)
{
	...
	pchar.quest.GothicAPSMoney.win_condition.l1.item = "Totem_0080";

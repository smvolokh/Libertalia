...
//////////////////////////////////////////// отгул у матросов ///////////////////////////////Rigel & Пират Либерталии /////////////////////////////////////////			

		case "Otgul_matrosov": // увольнительная
	Log_info("Теперь можно собирать матросов на корабль.");
	...
	SetTimerCondition("Otgul_matrosov_poka_na_susche", 0, 0, 1, false);
		...
		case "Otgul_matrosov_poka_na_susche": // увольнительная все время дает бонусы
	Log_info("Теперь можно собирать матросов на корабль.");
			pchar.Komanda_otdohnula = true;
			pchar.Komanda_gulaet = true;
...
/////////////////////// таймер тренировки Пират Либерталии 10.01.2020//////////////////////////////////////////////////////////////////////			
...
/////////////////////////////таймер разборки Пират Либерталии 10.01.2020//////////////////////////////////////////////////////////////////////////////			
...
/////////////////////////////таймер торжества Пират Либерталии 10.01.2020//////////////////////////////////////////////////////////////////////////////			
...
//////////таймер попойек в таверне Пират Либерталии 
26.02.2020 /////////////////////
/////////////////////////////////////////////////////////////////////////////			
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	Ship_PlayVictory("music_sea_songs", 125000);
		...
		case "sleep_in_tavern":
			...
			// Jason: если Мэри или Элиза в подругах и абордажниках - ночуем вместе
...
//			LAi_group_SetCheck("EnemyFight", "LandEnc_RapersAfrer");
		...
		case "PlaySex_2":
			...
			{
				LocatorReloadEnterDisable("SantLous_tavern", "reload1_back", false); //открыть таверну
				LocatorReloadEnterDisable("SantLous_tavern", "reload2_back", true); //закрыть комнату
				bDisableFastReload = false;//открыть переход
				DeleteAttribute(pchar, "GenQuest.CannotWait");
			...
			{
				LocatorReloadEnterDisable("Fishing_Settlement_tavern", "reload1", false); //открыть таверну
				LocatorReloadEnterDisable("Fishing_Settlement_tavern", "reload2", true); //закрыть комнату
				bDisableFastReload = false;//открыть переход
				DeleteAttribute(pchar, "GenQuest.CannotWait");
...
//////////////////////////////////////////////////////////////////////////////////////////////////
	...
	case "Sharlie_Start_1":
		...
		LocatorReloadEnterDisable("SantLous_town", "reload10", true);//закроем резеденцию Ли до поры до времени
                                         ...
                                         LocatorReloadEnterDisable("Beliz_Loguna_exit", "reload3", false);//закроем Теночтитлан до поры до времени  
		...
		// Addon-2016 Jason, французские миниквесты (ФМК)
	                  ...
	                  sld = CharacterFromID("Abuyin");//Даём нуду Абуину предсказателю
...
//-------------------------------------------против всех-----------------------------------------------------
		...
		case "ReturnShoreToNormal":
			...
			locations[FindLocation("Roseau_port")].DisableEncounters = false; //локацию в норму
		break;
...
sld.MultiShooter = 2.0;
	...
	//---------------------------------------Завещание Шарпа-------------------------------------------
		...
		case "Saga_HelenaIslaTesoro": // Элиза с ГГ на улице - подводим итоги Саги
			sld = characterFromId("Loxly");
			LAi_SetHuberType(sld);
		...
		case "Saga_HelenaBye": // расстаемся с Элизой по собственному решению, не стираем
			chrDisableReloadToLocation = true;//закрыть локацию
			sld = characterFromId("Helena");
			RemovePassenger(pchar, sld);
			LAi_SetActorType(sld);
		...
		case "Saga_HelenaOnIsland": // Элиза на Исла Тесоро
			sld = characterFromId("Helena");
	...
	// ------------------------------------ провалы Саги -------------------------------------------------
		case "Saga_FailCenturion": // утопили Центурион - Данни и Натана в Саванну-ла-Мар
			chrDisableReloadToLocation = false;//открыть локацию
			sld = characterFromId("Danielle");
...
///////////////////////////////Попойки в таверне и торжества во дворце //////////////////////////////////////////////////// Rigel & Пират Либерталии ////
		
		case "QPirPirPir1":
	Log_info("Эх, неплохо посидели... Ик!");
...
ChangeCharacterComplexReputation(pchar, "authority", 10); // авторитет в команде слегка увеличивается
	SetTimerCondition("Netotjig_v_taverne_1", 0, 0, 0, false);
		break;
		
		case "QPirPirPir2":
		Log_info("Эх, хорошо посидели... Ик!");
			PChar.GenQuest.VideoAVI = "Popoika";
			PChar.GenQuest.VideoAfterQuest = "Unifersal_end_video";
			AddCrewMorale(pchar, 15);
		AddCharacterExpToSkill(PChar, "Leadership", 400); 
			DoQuestCheckDelay("PostVideo_Start", 0.2);
			WaitDate("", 0, 0, 0, 6, 10); //крутим время
			pchar.GenQuest.PopoikaBonus = 5;
			ChangeOfficersLoyality("good_all", 3);
			ChangeCharacterComplexReputation(pchar, "authority", 15); // авторитет в команде слегка увеличивается
			pchar.reputation.fame = sti(pchar.reputation.fame) + 5; // известность самую малость повышается
	SetTimerCondition("Netotjig_v_taverne_2", 0, 0, 0, false);
		break;
		
		case "QPirPirPir3":
	Log_info("Эх, отлично посидели... Ик!");
			PChar.GenQuest.VideoAVI = "Popoika";
			PChar.GenQuest.VideoAfterQuest = "Unifersal_end_video";
		...
		AddCharacterExpToSkill(PChar, "Leadership", 600); 
			...
			ChangeCharacterComplexReputation(pchar, "authority", 20); // авторитет в команде слегка увеличивается
			pchar.reputation.fame = sti(pchar.reputation.fame) + 10; // известность самую малость повышается
	SetTimerCondition("Netotjig_v_taverne_3", 0, 0, 0, false);
		break;		
		
		case "QPirPirPir4":
		Log_info("Эх, чудно посидели... Ик!");
			PChar.GenQuest.VideoAVI = "Popoika";
			PChar.GenQuest.VideoAfterQuest = "Unifersal_end_video";
		...
		AddCharacterExpToSkill(PChar, "Leadership", 800); 
			...
			ChangeCharacterComplexReputation(pchar, "authority", 25); // авторитет в команде слегка увеличивается
			pchar.reputation.fame = sti(pchar.reputation.fame) + 15; // известность самую малость повышается
	SetTimerCondition("Netotjig_v_taverne_4", 0, 0, 0, false);
		break;	
	
		case "QPirPirPir5":
		Log_info("Эх, замечательно посидели... Ик!");
			PChar.GenQuest.VideoAVI = "Popoika";
			PChar.GenQuest.VideoAfterQuest = "Unifersal_end_video";
		...
		AddCharacterExpToSkill(PChar, "Leadership", 1000); 
			...
			ChangeCharacterComplexReputation(pchar, "authority", 30); // авторитет в команде слегка увеличивается
			pchar.reputation.fame = sti(pchar.reputation.fame) + 20; // известность самую малость повышается
	SetTimerCondition("Netotjig_v_taverne_5", 0, 0, 0, false);
		break;

		case "Torjestva_dvorec": // дворцовые торжества
			PChar.GenQuest.VideoAVI = "Torjestva_dvorecovie";
			PChar.GenQuest.VideoAfterQuest = "Unifersal_end_video";
			DoQuestCheckDelay("PostVideo_Start", 0.2);
			WaitDate("", 0, 0, 0, 4 + rand(12), 10); //крутим время
			pchar.GenQuest.PopoikaBonus = 30;
			pchar.reputation.fame = sti(pchar.reputation.fame) + 30; // известность повышается
			ChangeCharacterComplexReputation(pchar, "nobility", 5); // мнение в обществе улучшается
			ChangeCharacterComplexReputation(pchar, "authority", 40); // авторитет повышается 
			ChangeOfficersLoyality("bad_all", 3);
AddCharacterExpToSkill(PChar, "Leadership", 1500); // харизма повышается 
AddCrewMorale(pchar, -20); // мораль снижается типо пижон, все дела
SetTimerCondition("Vremya neumalimo Otjig", 0, 0, 30 + rand(10), true);
			Log_info("Торжества во дворце губернатора и впрямь оказались на высшем уровне!");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////		
		...
		case "Caleuche_ReloadCoast": // на берег
			...
			pchar.Ship.name = "Плот";
			pchar.Ship.Cannons.Type = CANNON_TYPE_CANNON_LBS3;
			SetBaseShipData(pchar); // на плот
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// ФМК-Гваделупа
		...
		case "FMQP_AfterFightInTown": // 
			...
			SetFunctionTimerCondition("FMQP_SantoDomingoOver", 0, 0, 7, false);
		break;
		...
		case "FMQL_ClearBeach": // чистка квеста
			...
			LAi_ActorGoToLocation(sld1, "reload", "sea", "none", "", "", "", 10.0); // Пират Либерталии фикс бага с затиранием Джонни. Прогон 1. 18.07.2020.
		AddCharacterExpToSkill(pchar, "Leadership", 100);
		...
		case "FMQL_AfterBattleOnBeach": // закончили бой с кодлой Валинье
			...
			LAi_KillCharacter(sld1);			
			LAi_SetActorType(sld1);
			LAi_ActorGoToLocation(sld1, "reload", "sea", "none", "", "", "", 10.0); // Пират Либерталии фикс бага с затиранием Джонни. Прогон 2. 18.07.2020.
			...
			LAi_KillCharacter(sld1);			
			LAi_SetActorType(sld1);
			LAi_ActorGoToLocation(sld1, "reload", "sea", "none", "", "", "", 10.0); // Пират Либерталии фикс бага с затиранием Джонни. Прогон 3. 18.07.2020.
			ChangeCharacterComplexReputation(pchar, "authority", 10);
		break;
...
FantomMakeCoolFighter(sld, 20, 50, 50, "blade_15", "", "", 100);
	...
	ChangeCharacterAddressGroup(sld, "LaVega_TwoFloorHouse", "goto", "goto3");
		...
		case "Mtraxx_MeridaGotoRiver_3": // Тагофа ведет нас к реке
			chrDisableReloadToLocation = true;
			sld = characterFromId("Tagofa");
			ChangeCharacterAddressGroup(sld, "Maracaibo_jungle_05", "reload", "reload3_back");
			LAi_SetActorType(sld);
			LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
			LAi_ActorGoToLocation(sld, "reload", "reload1_back", "none", "", "", "OpenTheDoors", -1);
		...
		case "Mtraxx_MeridaRiverTrip": // плывем по реке на лодках
			...
			LocatorReloadEnterDisable("Shore106", "boat", false); // выпустим в море
		break;
	...
	PChar.nation	= PIRATE;
		...
		case "Mtraxx_CartahenaFortFirstBattle": // бой в цитадели форта
			PlaySound("interface\_GTBoard2.wav");
...
FantomMakeCoolFighter(sld, iRank, 60, 60, LinkRandPhrase("blade_16","blade52","newblade15"), "pistol11", "bullet", 150);
	...
	FantomMakeCoolFighter(sld, iRank, 60, 60, LinkRandPhrase("blade_03","blade_05","blade_07"), "pistol1", "bullet", 150);
		...
		case "Mtraxx_RetributionPromice": // 
			PlaySound("ambient\church\zvon.wav");
			Log_Info("Я, " + GetFullName(pchar) + ", вольный капитан и мореплаватель, клянусь перед собой и своей совестью:");
...
/// Jason ----------------------------------------------------------- На службе Отечеству ------------------------------------------------------------------
		...
		case "GoldenGirl_SPFinal": // завершение варианта Кортни в Форт де Франс
			chrDisableReloadToLocation = false;
...
// Jason -------------------------------------------------- Долго и счастливо ----------------------------------------------------------------
		...
		break;
			...
			//--> ---- блок празднования в таверне Форт де Франс-------------------
		...
		case "LongHappy_InSPcabin": //
			// нормализуем таверну Форт де Франс
			for (i=1; i<=10; i++)
			{

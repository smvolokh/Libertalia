	...
	switch (Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (CheckAttribute(pchar, "questTemp.Sharlie.Citcount") && npchar.city == "FortFrance")
			{
				...
				link.l1 = "Здравствуйте, Ваша Светлость. Я разыскиваю Мишеля де Монпе. Не могли бы вы посодействовать в моих поисках?";
                link.l1.go = "Sharlie";
			...
			if (CheckAttribute(pchar, "questTemp.Sharlie.Lock") && npchar.city == "FortFrance")
			{
				...
				{
					dialog.text = "А, это вы, месье... Видимо, я должен извиниться за то, что так круто с вами обошелся. Но поймите и меня тоже, я забочусь о нашей национальной безопасности...";
			...
			else
			{
                ...
                {//->
                    ...
                    else
                    {
						...
						{
							link.l15 = UpperFirst(GetAddress_FormToNPC(NPChar)) + ", я обнаружил" + GetSexPhrase("", "а") + " банду контрабандистов, чьего главаря недавно поймала стража. Они планируют вызволить его. Их корабль '" + PChar.GenQuest.JusticeOnSale.ShipName + "' стоит " + XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Dat") + ".";
						...
						{
							link.l16 = "Я прибыл" + GetSexPhrase("", "а") + " для участия в регате. Вот моё приглашение.";
							link.l16.go = "Regata";
						}
						...
						if (CheckAttribute(pchar, "questTemp.Patria.Governor") && npchar.city == "Marigo")
						{
							link.l10 = "Я приехал ненадолго. Рад, что у вас всё в порядке.";
							link.l10.go = "exit";
						}
						else 
						{
							if (CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && npchar.nation == FRANCE)
							{
								link.l10 = "Не надо. Я приехал ненадолго. Рад, что у вас всё в порядке.";
								link.l10.go = "exit"; 
							}
							else 
							{
								if (CheckAttribute(pchar, "questTemp.Patria.GenGovernor"))
								{
									link.l10 = "Не извольте беспокоиться, это визит вежливости, я не мог, посетив вашу колонию, не зайти к вам.";
									link.l10.go = "exit"; 
								}
								else
								{
        ...
        case "Card_Game":
            if (!CheckNPCQuestDate(npchar, "Card_date_Yet") || sti(PChar.rank) < 4 || isBadReputation(pchar, 50) || GetCharacterSkillToOld(pchar, SKILL_LEADERSHIP) < rand(5))
            {
				...
				if (pchar.model == "Sharle_6") dialog.text = "Прошу меня извинить, монсеньор, но сейчас у меня нет на это времени. Может быть, в другой раз."; // Jason НСО
                else dialog.text = "Я не буду играть в азартные игры с разными проходимцами!";
...
//navy <--
		...
		case "CAPTURE_Main":
			...
			{
                dialog.Text = "Аргх! Какого черта вы творите этот беспредел? С каких это пор каперы атакуют Тортугу?";
                Link.l1 = "По приказу Франсуа Ловинье де Пуанси ты, Левассер, смещен с поста губернатора Тортуги и приговорен к смерти. А я - исполнитель этого приговора!";
                Link.l1.go = "levasser";
				break;
			}
			// <-- Левассер
		...
		// --> Левассер
		case "levasser":
            dialog.Text = "Пуанси?! Проклятый старый хрыч! С ним я еще разберусь, а тебе пришел конец, негодяй! Я лично позабочусь о том, чтобы ты не вышел отсюда живым!";
...
//<-- осады homo
		...
		case "Sharlie":
			...
			link.l1 = "Я его друг. Его брат попал в беду и я должен сообщить ему об этом...";
			link.l1.go = "Sharlie_1";			
		break;
		
		case "Sharlie_1":
			dialog.text = "Вот как? Что-то я не припомню никакаких друзей здесь у месье де Монпе... Вот что, сударь: вы мне кажетесь очень подозрительным, и я вас арестую для дальнейшего разбирательства. Лейтенант! Взять этого человека под стражу!";
		...
		case "JusticeOnSale_1":
			...
			link.l1 = GetSexPhrase("Рад был помочь", "Рада была помочь") + ", разрешите откланяться?";
		...
		case "Andre_Abel_Quest_Hol_Gov_Node_3":
			dialog.text = "Что?! Оставить Филипсбург?! Ах, английские собаки! Мало им пол архипелага, так на все карибские острова рот разинули! Схватить "+ GetSexPhrase("этого проходимца","эту мерзавку") +"!";
		...
		case "Andre_Abel_Quest_Hol_Gov_Node_9":
			...
			dialog.text = "Договориться? Хм... ну, если речь идёт о сумме, скажем " + iSumm + " песо, то я готов продолжить диалог.";
	...
	break;
		...
		LAi_LocationFightDisable(&Locations[FindLocation("Santiago_town")], false); // разрешить бить морды в Сантьяго
   		NextDiag.CurrentNode = NextDiag.TempNode;
		DialogExit();
		break;
...
string SelectPassText()
{
	...
	switch (drand(5))
	{
		case 0: sText = "Этот иуда, воспользовавшись своим положением, выдал за деньги важную государственную тайну враждебной державе."; break;
		case 1: sText = "Этот негодяй подло убил своего родственника, известного и знатного человека, ради наследства."; break;
		case 2: sText = "Этот гнусный негодяй нанёс тяжкое оскорбление моей семье и мне лично."; break;
		case 3: sText = "Этот вороватый пройдоха выкрал из казны нашего города приличную сумму денег."; break;
		case 4: sText = "Этот негодяй виновен в смерти почти десятка наших граждан."; break;
		case 5: sText = "Этот пройдоха, используя свои связи, обманом завладел одним купеческим судном, украл его и продал на соседней верфи."; break;
	}
	return sText;
}
...
string SelectFugitiveText()
{
	...
	switch (drand(5))
	{
		case 0: sText = "один из младших офицеров нашего гарнизона бросил службу и скрылся на купеческом корабле из нашей колонии."; break;
		case 1: sText = "один из наших офицерских чинов дезертировал, прямо во время несения службы, и сбежал на корабле контрабандистов."; break;
		case 2: sText = "один из наших морских офицеров, служивших на патрульном бриге, по непонятным причинам дезертировал, угнал баркас и скрылся."; break;
		case 3: sText = "один тюремный стражник по необъяснимой причине бросил службу и сбежал из нашей колонии на частном судне."; break;
		case 4: sText = "один солдат нашего гарнизона бросил свой пост, прямо в то время, когда нёс караул, отправился в бухту и покинул наши места на неопознанном корабле."; break;
		case 5: sText = "один наш военный офицер, капитан патрульного люггера, бросил своё судно прямо в море и скрылся в неизвестном направлении на рыбацком баркасе."; break;
	}
	return sText;
}

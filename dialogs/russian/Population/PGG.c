...
link.l1 = "Приветствую, капитан! Мое имя - "+GetFullName(pchar)+". У меня к Вам деловое предложение!";
					link.l1.go = "march";
...
sld = GetRealShip(sti(NPChar.Ship.Type));
Dialog.Text = "Приветствую Вас, коллега! Приятно, знаете ли, видеть преуспевающего и достойного капитана! Позвольте представиться - " + GetFullName(NPChar) + ". Я капитан " + xiStr(sld.BaseName + "PGG") + " '" + NPChar.Ship.Name + "', под флагом " + NationNameGenitive(sti(NPChar.nation)) + ". Чем могу быть полезен?";
link.l1 = "Приветствую, капитан! Мое имя - "+GetFullName(pchar)+". Я капитан корабля '" + PChar.Ship.Name + "' под флагом " + NationNameGenitive(sti(PChar.nation)) + ". У меня к Вам деловое предложение!";
					link.l1.go = "march";
					link.l2 = "О, приветствую Вас! Мое имя - "+GetFullName(pchar)+". Я капитан корабля '" + PChar.Ship.Name + "' под флагом " + NationNameGenitive(sti(PChar.nation)) + ". Я очень рад нашей встрече. Слыхал я, что Вы бывалый и опытный моряк, не желаете пойти офицером ко мне на корабль? В деньгах не обижу.";
	link.l2.go = "officer";
	link.l3 = "Приветствую, коллега! Мое имя - "+GetFullName(pchar)+". Я капитан корабля '" + PChar.Ship.Name + "' под флагом " + NationNameGenitive(sti(PChar.nation)) + ". Я много слышал о Вас... Судя по слухам, Вы настоящая гроза всего архипелага! Думаю, что если бы мы объединили наши усилия, то вместе мы смогли бы гораздо больше... Что скажите, капитан?";
		...
		link.l8 = "Привет, приятель! Мое имя - "+GetFullName(pchar)+". Я капитан корабля '" + PChar.Ship.Name + "' под флагом " + NationNameGenitive(sti(PChar.nation)) + ". Я просто решил поздороваться. Бывай!";
			...
			else
			{
				...
				if (sti(NPChar.nation) != PIRATE && GetNationRelation2MainCharacter(sti(NPChar.nation)) == RELATION_ENEMY)
				{
					...
					link.l1 = "У меня к Вам деловое предложение, капитан!";
					link.l1.go = "march";
...
Dialog.Text = "Если вы читаете это, значит, вы поймали баг ;) Сообщите об этом Navy, подробно, как вы его поймали :)";
		...
		{
			Dialog.Text = RandPhraseSimple("Я Вам уже все сказал на сегодня!", "Мне казалось, что мы все обсудили.");
...
//			link.l1 = "Выкладывай сейчас, некогда мне по гостям расхаживать.";
	...
	case "Quest_1_Work_2":
		Dialog.Text = "Я же сказал, что жду вас на корабле!";
		link.l1 = "А... конечно...";
...
//			Dialog.Text = "Н-да, связался я с тобой. Все погубил. Ты теперь мне должен неустойку в размере " + FindRussianMoneyString(sti(PChar.GenQuest.PGG_Quest.FailedPaySum)) + ".";
		...
		{
			PChar.GenQuest.PGG_Quest.Stage = -1;
			Dialog.Text = "Н-да, связался я с тобой. Все погубил"+ GetSexPhrase("","а") +". Не умеешь -  не берись, и других не втягивай.";
		...
		if (bOk || CheckAttribute(PChar, "GenQuest.PGG_Quest.Ok"))
		{
			...
			link.l1 = PCharRepPhrase(RandPhraseSimple("Клянусь дьяволом, ты за это заплатишь!", 
					...
					RandPhraseSimple("Ничего личного - это деловой подход.", 
						"Кто сильнее, тот и прав. Ничего личного, капитан."));
...
/*		if (PChar.Location == "Ship_deck")
	...
	case "Quest_1_SharePrise":
		...
		link.l1 = PCharRepPhrase(RandPhraseSimple("Справедливо, разрази меня гром!", "Да, все точно, разрази меня гром!"), 
			RandPhraseSimple("Ваши расчеты правильны. Я соглас"+ GetSexPhrase("ен","на") +".", "В дележе вы безупречны."));
		link.l1.go = "Exit_Quest_1_End";
		link.l2 = PCharRepPhrase(RandPhraseSimple("Ты еще требуешь свою долю? Да пропади ты пропадом, сукин сын!", 
						"Долю? Здесь каждый сам за себя, и если тебе ничего не досталось, то это твои проблемы!"), 
					RandPhraseSimple("Не думаю, что вы на что-то можете рассчитывать.", 

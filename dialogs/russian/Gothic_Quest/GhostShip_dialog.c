...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "AfterMeet":
			...
			if (GetCharacterItem(Pchar, "DestinyGiftPearl") >= 666 && CheckAttribute(pchar , "GenQuest.GhostShip.NeedCoins"))
			{
		...
		case "quest_coin_2":
			...
			Link.l1.go = "Exit";
			TakeNItems(pchar, "DestinyGiftPearl", -666);
		...
		case "GhostCapt_Prize1":
			dialog.text = "Да... немногие, даже из тех, кто видел меня живым, узнали бы сейчас старину Дэйви Джонса...";
			link.l1 = "Так ты и есть легендарный капитан ''Летучего Голландца'' Дэйви Джонс?!";
		...
		case "GhostCapt_Prize4":
			...
			link.l3 = "Благодарю, но мой кораблик меня вполне устраивает! Потопил же он твоего хвалёного ''Голландца''.";
			link.l3.go = "GhostCapt_noupgrade";
		break;
		...
		case "GhostCapt_upgrade1":
			...
			link.l1 = "Что так и сказать, мол привет тебе от покойного капитана ''Летучего Голландца'', мистера Дэйви Джонса?";
		...
		case "GhostCapt_LastDialog":
			...
			link.l1 = " Я " + GetSexPhrase("пришел","пришла") + " освободить тебя и твоих людей от проклятия. Твоё время вышло - пора на покой, гроза морей и капитан призрака Дэйви Джонс.";

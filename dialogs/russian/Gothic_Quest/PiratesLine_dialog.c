void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		// -------------------------------------- квест №1 -------------------------------------------------
		//первая встреча с посыльным Моргана в Вестпунте
		case "PiratesMan1":
			if (pchar.questTemp.State == "" || sti(pchar.questTemp.CurQuestNumber) > 8) 
			{
		...
		case "PiratesMan1_2":
			...
			FreeSitLocator("Vestpunt_tavern", "sit3");
			LAi_SetActorType(npchar);
			LAi_ActorRunToLocation(npchar, "reload", "reload5", "Vestpunt_tavern", "sit", "sit3", "PiratesLine_toTavern", -1);
		...
		break;
		case "Tavern_3":
			dialog.text = "Да ничего особенного. Зовут Джон Болтон, будет ждать тебя в порту Вестпунта. Ты только лишнего не болтай!";
		...
		case "Tavern_5":
			...
			sld = GetCharacter(NPC_GenerateCharacter("QuestPirate2", "officer_4", "man", "man", 20, PIRATE, -1, true, "quest"));
			sld.name = "Джон";
			sld.lastname = "Болтон";
			sld.rank = 20;
			sld.city = "Vestpunt";
			sld.location	= "Vestpunt_port";
		...
		case "Tavern_again":
			dialog.text = "Джон ждет тебя в порту Вестпунта. Не тяни с этим!";
			link.l1 = "Хорошо...";
			link.l1.go = "exit";
			NextDiag.TempNode = "Tavern_again";
		break;
		case "Tavern_over":
			...
			AddDialogExitQuestFunction("PiratesLine_q1_tavernEnd");
		break;
		//базар с Джоном Болтоном в порту Плайя-Калки
		case "JohnBolton":
		...
		// -------------------------------------- квест №2 -------------------------------------------------
		case "EdwardLoy":
	                                                PlaySound("VOICE\Russian\gotica\DAR\DAR_00.wav");
			dialog.text = "Ты ко мне, "+ GetSexPhrase("приятель","подруга") +"? Хочешь затянуться?";
			link.l1 = "";
			link.l1.go = "EdwardLoy_01";
		break;

		case "EdwardLoy_01":
	                PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_HALLO_15_00.wav");
			dialog.text = "";
			link.l1 = "А что ты куришь?";
			link.l1.go = "EdwardLoy_02";
		break;

		case "EdwardLoy_02":
	                PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_DUDIEB_03_01.wav");
			dialog.text = "";
			link.l1 = "";
			link.l1.go = "EdwardLoy_03";
		break;

		case "EdwardLoy_03":
	                PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_HALLO_JA_03_01.wav");
			dialog.text = "Неплохо, да?";
			link.l1 = "";
			link.l1.go = "EdwardLoy_04";
		break;

		case "EdwardLoy_04":
	                                PlaySound("VOICE\Russian\gotica\DAR\DAR_02.wav");
			dialog.text = "Вот, затянись...";
			link.l1 = "";
			link.l1.go = "EdwardLoy_05";
		break;

		case "EdwardLoy_05":
	                                PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_HALLO_JA_15_00.wav");
			dialog.text = "";
			link.l1 = "Конечно, давай.";
			link.l1.go = "EdwardLoy_06";
		break;

		case "EdwardLoy_06":
			Log_Info("+ энергия");
			pchar.questTemp.Gothicfennisgrasstotal = true;
	                                PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_HALLO_JA_15_02.wav");
			dialog.text = "";
			link.l1 = "Где ты взял эту траву?";
			link.l1.go = "EdwardLoy_07";
		break;

		case "EdwardLoy_07":
	                                PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_HALLO_JA_03_03.wav");
			dialog.text = "Хм, у меня свои источники!";
			link.l1 = "";
			link.l1.go = "EdwardLoy_08";
		break;

		case "EdwardLoy_08":
	                               PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_DUDIEB_03_01.wav");
			dialog.text = "";
			link.l1 = "Круто... Кстати, тебе привет от Генри Моргана.";
			link.l1.go = "EdwardLoy_09";
		break;


		case "EdwardLoy_09":
			dialog.text = "...От Генри Моргана? И что понадобилось от меня Генри?";
			link.l1 = "Генри очень по тебе соскучился, просит навестить при случае.";
			link.l1.go = "EdwardLoy_010";
		break;
 		...
 		case "EdwardLoy_010":
			...
			LAi_ActorTurnToCharacter(sld, pchar);
   			AddDialogExitQuest("EdwardLoy_BlackLabel");
			NextDiag.CurrentNode = "EdwardLoy_2";
			DialogExit();
		break;
		...
		case "EdwardLoy_3":
			dialog.text = "Дьявол! Чёрная Метка. Я так и знал... А если я её не возьму?";
			link.l1 = "А если ты её не возьмёшь, то я засуну её тебе в глотку.";
			link.l1.go = "EdwardLoy_4";
		                     RemoveItems(PChar, "BlackLabel", 1);
		break;
		case "EdwardLoy_4":
	                                                PlaySound("VOICE\Russian\gotica\DAR\DAR_01.wav");
			dialog.text = "Раслабся, я слишком обкурился что бы драться с тобой! Ты сделал"+ GetSexPhrase("","а") +" свое подлое дело, теперь убирайся отсюда!";
		...
		case "EdwardLoy_again":
	                                PlaySound("VOICE\Russian\gotica\DAR\DAR_04.wav");
			dialog.text = "Мне плевать. Ты еще здесь?";
			link.l1 = "Уже ухожу...";
			link.l1.go = "exit";
			NextDiag.TempNode = "EdwardLoy_again";
		break;
		...
		case "SeaWolf":
	                                      PlaySound("VOICE\Russian\gotica\DAR\DAR_03.wav");
			dialog.text = "Иногда я позволяю всяким мстительным болотным наркоманам задать мне взбучку.";
			link.l1 = "";
			link.l1.go = "SeaWolf_1_1";
		break;

		case "SeaWolf_1_1":
	                                      PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_WOPAKET_15_03.wav");
			dialog.text = "";
			link.l1 = "Может тебе ещё нужна хорошая встряска?";
			link.l1.go = "SeaWolf_1_2";
		break;

		case "SeaWolf_1_2":
	                                      PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_ORCRING_NECKEN_03_03.wav");
			dialog.text = "Ты здесь всего пару дней и уже задрал нос выше облаков!";
			link.l1 = "";
			link.l1.go = "SeaWolf_1_3";
		break;

		case "SeaWolf_1_3":
	                                      PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_ORCRING_03_03.wav");
			dialog.text = "Если я смогу принести доказательства, что действительно крутой парень, возможно я смогу стать одним из телохранителей Ли!";
			link.l1 = "";
			link.l1.go = "SeaWolf_1_4";
		break;

		case "SeaWolf_1_4":
	                                      PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_ORCRING_NECKEN_15_00.wav");
			dialog.text = "";
			link.l1 = "Крутой парень? Ты?";
			link.l1.go = "SeaWolf_1_5";
		break;

		case "SeaWolf_1_5":
	                                PlaySound("VOICE\Russian\gotica\DAR\DAR_05.wav");
			dialog.text = "Ох, ладно, заткнись - ты то вообще кто такой? Да что ты о себе возомнил"+ GetSexPhrase("","а") +"?! Думаешь, ты так"+ GetSexPhrase("ой","ая") +" всемогущ"+ GetSexPhrase("ий","ая") +"?! Ты прост"+ GetSexPhrase("ой порученец","ая наемница") +" Моргана, не более того!";
			link.l1 = "";
			link.l1.go = "SeaWolf_1_6";
		break;

		case "SeaWolf_1_6":
	                                PlaySound("VOICE\Russian\gotica\DAR\DAR_06.wav");
			dialog.text = "Какой-то простофиля из города - тебе вообще ничего не светит!";
		...
		case "SeaWolf_2":
			...
			link.l1 = "Если это может потешить твоё самолюбие перед смертью, то Морган поставил за тебя пятьдесят тысяч песо.";
		...
		case "SeaWolf_4":
			...
			link.l1 = "Заткнись, Лоу! В своих делах с Морганом я разберусь сам"+ GetSexPhrase("","а") +", когда придёт время. И ты умрёшь не за пятьдесят тысяч. Ты умрёшь за свою подлость. Торус просил меня сделать это бесплатно... У меня есть какие-нибудь причины оставить тебе жизнь?";
			link.l1.go = "SeaWolf_5";
		break;
		case "SeaWolf_5":
	                   PlaySound("VOICE\Russian\gotica\DAR\DAR_07.wav");
		dialog.text = "Кого ты хочешь напугать своей магической чушью - только не меня. Ха! Представляю рожу Торуса, когда он узнал, что корсарский бриг его - тю-тю...";
			link.l1 = "";
			link.l1.go = "SeaWolf_6";
		break;

		case "SeaWolf_6":
	                                PlaySound("VOICE\Russian\gotica\DAR\DIA_DAR_ORCRING_NECKEN_NO_15_00.wav");
			dialog.text = "";
			link.l1 = "У меня нет времени на эту чушь. Представил? Ну, тогда с Богом...";
			link.l1.go = "SeaWolf_7";
		break;

		case "SeaWolf_7":
	                   PlaySound("VOICE\Russian\gotica\DAR\DAR_08.wav");
		...
		//Эйвори вместо ГГ
		case "PQ5Hero":
			dialog.text = "А ты ещё кто такой? Орри, ты третьего звал?";
			link.l1 = "Никто не звал, ибо я сам по себе!";
			link.l1.go = "PQ5Hero_1";
		break;
		case "PQ5Orry":
			dialog.text = "Я похож на шутника? Ты ещё кто такой?";
			link.l1 = "Я вольный капитан " + GetFullName(pchar) + "!";
			link.l1.go = "PQ5Hero_1";
		break;
		case "PQ5Hero_1":
            DialogExit();
		...
		case "PQ5Aivory":
			dialog.text = "Тебя не звали! Вали от сюда пока ветер без сучков!";
		...
		case "JohnLids":
			...
			link.l1 = "Здравствуй, Джон. Меня послал к тебе Торус.";
		...
		case "LeonCapitain":
	                                PlaySound("VOICE\Russian\GOTHIC\SLD_02.wav");

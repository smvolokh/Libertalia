...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "facio":
			...
			link.l1 = "Чего? Что ты там надумал? Давай выкладывай, не трать мое время попусту!";
			link.l1.go = "facio_1";
		...
		case "facio_1":
			...
			link.l1 = "Заработать я всегда хочу. Однако все зависит от того, что ты предложишь: я не на все дела подписываюсь, даже на сулящие приличные барыши.";
			link.l1.go = "facio_2";
		break;
		
		case "facio_2":
			dialog.text = "Дело, конечно, не очень чистое, но зато хорошо наживемся... и накажем одного обнаглевшего типа. Он не обеднеет, у него и так денег полно. Все с ним делятся - пора бы и ему поделиться. Теперь скажи: как ты переносишь ром? Много ли сможешь выпить, прежде чем свалишься под стол?";
			link.l1 = "Ну, когда как. Не знаю, как-то не имею привычки напиваться в стельку... Ты когда-нибудь дойдешь до сути своего предложения, или будешь без конца кота за хвост тянуть?";
			link.l1.go = "facio_3";
		break;
		
		case "facio_3":
			dialog.text = "Я уже начал излагать суть, просто ты пока не понял. Ладно. Объясняю конкретно. Есть у нас на Острове один мерзкий толстяк - Джузеппе Фацио. Пройдоха, каких свет не видывал. Обдирает и нарвалов, и ривадос, и прочих жителей. Я давно к нему присматриваюсь, и кое-что вызнал\nУ него в каюте на 'Каролине' есть два сундука: в одном он хранит всякие бумаги, а во втором - золотые и серебряные монеты. Оба сундука запираются на ключ, причем каждый - на свой. Толстяк никогда не расстается с этими ключами...";
		...
		case "facio_4":
			dialog.text = "С палубы 'Каролины' в каюту ведут две двери. Одна из них всегда заперта. Так вот, ключ, отпирающий сундук с деньгами, открывает также и эту дверь. Уж не знаю, зачем так Фацио придумал, но это факт. Достаточно иметь всего один ключ, чтобы и проникнуть в каюту, и отпереть сундук. Логичный вопрос: как достать ключ? Так вот - у меня есть план\nОчень хороший план - в случае успеха Фацио даже не поймет, как это случилось, и кто виноват. Спишут на таинственного воришку. Но для выполнения задуманного мне нужен помощник, причем с крепкой головой. Ты меня слушаешь?";
			link.l1 = "Очень внимательно. Продолжай.";
			link.l1.go = "facio_5";
		break;
		
		case "facio_5":
			dialog.text = "Фацио любит посидеть в таверне у Санчо и хорошенько выпить, однако его туша весит столько, что никакой ром не берет. Поэтому он и придумал одно развлечение: пить на спор, кто кого перепьет. Если ты предложишь ему это - он обязательно согласится\nИ вот тут-то и начинается самое веселое. Ты заключаешь с ним пари, вы вечером идете к Санчо, покупаете ром и начинаете пить кружку за кружкой, пока кто-то из вас первым не свалится. Если отрубишься ты - план провалится, ибо я не смогу все сделать сам\nПоэтому я и спросил - крепкая ли у тебя голова. Если же тебе удасться довести Фацио до состояния поросячьего визга, считай - дело в шляпе. Я все время буду рядом. Как только он вырубится - я вытащу у него ключ от 'Каролины'\nЗатем мы отведем толстяка к Санчо на кровать наверх, и пока он будет дрыхнуть - заберемся в каюту 'Каролины', откроем сундук, заберем содержимое, закроем, и я верну ключ Фацио на место - ему в карман. Толстяк так и не поймет, как его ограбили\nДостать и вернуть ключ я беру на себя, твоя задача - напоить Фацио и пробраться в каюту. Я буду стоять на стреме, пока ты будешь копаться в его сундуке, и в случае чего - отвлеку внимание, а ты смоешься через кормовую галерею\nСодержимое сундука поделим пополам. Это будет справедливо. Ну как, готов попробовать?";
		...
		case "facio_6":
			dialog.text = "Я рад, что не ошибся в тебе, парень. Теперь ступай к Фацио и договорись о встрече в таверне вечером, после девяти. Надеюсь, ты перепьешь этого борова. Удачи и до встречи вечером!";
		...
		case "result_0":
			dialog.text = "Все, эта жирная свинья дрыхнет наверху. Ключ у меня. Ты держался молодцом - похоже, у тебя вообще ни в одном глазу!";
			link.l1 = "Ну, не скажи. Давно я так не накачивался. Слегка пошатывает... Ну что, идем?";
			link.l1.go = "result_win";
		break;
		
		case "result_1":
			dialog.text = "Все, эта жирная свинья дрыхнет наверху. Ключ у меня. Ты держался молодцом - сумел-таки перепить этого борова...";
			link.l1 = "Сумел, но я давненько так не нажирался. Штормит сильно, голова кругом... Ну что, идем?";
			link.l1.go = "result_win";
		break;
		
		case "result_2":
			dialog.text = "Эх, дружище, все-таки не потянул ты против этого толстяка... Жаль. Ну, да что поделаешь - хоть попытались...";
			link.l1 = "Ч-черт, воз-зьми, он просто какая-то... бездон-н-ная боч-чка. Аргх, мне надо пос-пать... Все, про-валивай! Ик!..";
			link.l1.go = "result_fail";
		break;
		
		case "result_win":
			dialog.text = "Да. Идем! Следуй за мной к 'Каролине'! Ни на что не отвлекайся, времени в обрез!";
			link.l1 = "Пошли!";
		...
		case "result_win_2":
			...
			dialog.text = "Пришли. Держи ключ. Нужный сундук - слева от входа. Быстро перекладывай содержимое себе и линяй. Не забудь запереть его снова. Если увижу что подозрительное - постучу тебе в дверь три раза. В этом случае все бросай и беги через кормовую галерею.";
		...
		case "result_grabbing":
			...
			if (iPeso <= 0) sPeso = "ни одного песо";
			else sPeso = ""+FindRussianMoneyString(iPeso)+"";
			if (iDubl <= 0) sDubl = "ни одного дублона";
			else sDubl = ""+iDubl+" дублонов";
			if (iPeso <= 10000 && iDubl <= 10) sTotalTemp = "Что за жалкие крохи... И ради этого мы рисковали? Эх... Ладно, делим, что есть.";
			else
			{
				if (iPeso <= 50000 && iDubl <= 100) sTotalTemp = "Я думал, там в разы больше... Риск того не стоил. Эх, ладно, делим, что есть.";
				else
				{
					if (iPeso <= 100000 && iDubl <= 300) sTotalTemp = "Хм. Неплохо. Хотя должно было быть больше по моим расчетам. Ладно, давай делить.";
					else 
					{
						if (iPeso <= 150000 && iDubl <= 500) sTotalTemp = "Отлично! Мы с тобой ловко сработали, напарник! Это целая куча монет! Давай делить!";
						else
						{
		...
		case "grabbing_0":
			dialog.text = "Не может быть! Получается, все было зря... Наверное, этот гад переложил свою заначку во второй сундук. Эх! Ладно, давай ключ, отнесу его этому пьянице, пока он не хватился. Бывай, приятель...";
			link.l1 = "...";
...
//--------------------------------------- блок вопросов и ответов ---------------------------------------------
		case "int_quests":
			...
			{
				link.l2 = "Ты что, постоянно пьешь? Где же ты находишь деньги на выпивку?";
		...
		case "ansewer_2":
			dialog.text = "Да, я пью... Пью, черт возьми, ибо больше мне делать нечего... Пойми, дружище, я конченый человек. И счастье у меня осталось одно - бутылка рома. И я буду пить. А деньги... хи-хи, а их много на ром и не надо. Да и покупать мне его тоже не надо\nЕсть тут одно местечко, где рома много, но туда надо добраться, поэтому я покупаю ром у Санчо только тогда, когда не в состоянии доплыть... ик! Т-сс, нет, куда - не скажу, хи-хи, даже и не спрашивай...";
			link.l1 = "Ясно...";
			link.l1.go = "int_quests";

...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    switch (Dialog.CurrentNode)
	{
		...
		case "Mushket_stid_4":
			dialog.text = "Сто тысяч серебром, приятель! Сто тысяч серебром - и прямо сейчас.";
...
void SetMushketCapitainInWorld()
{
	...
	FantomMakeCoolSailor(sld, SHIP_ARMEDBRIG, "Стрела", CANNON_TYPE_CULVERINE_LBS18, 50+rand(50), 50+rand(50), 50+rand(50));

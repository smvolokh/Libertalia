...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	// ==> Мистер и Мисси Смит
			...
			{
                link.l1 = "Мне нужен банкир мистер Смит, он должен был снять комнату в вашей таверне на втором этаже. Могу я поговорить с ним?";
                link.l1.go = "Smithy_1";
			}
			...
			{
                link.l1 = "Мне нужен банкир мистер Смит, он должен быть здесь. Могу я поговорить с ним?";
        ...
        //********************** Мистер и Мисси Смит **********************
                                                         case "Smithy_1":
			dialog.text = "О, мистер Смит! Конечно, вы можете найти его наверху. Кстати говоря, он, кажется, ждёт вас. Вы ведь капитан " + GetFullName(pchar) + ", верно? Он ждёт вас.";
			link.l1 = "Да, " + GetFullName(pchar) + ". Спасибо за помощь. Всего хорошего.";
			link.l1.go = "exit";
...
LocatorReloadEnterDisable(npchar.city + "_tavern", "reload2_back", false);
			...
			dialog.text = "О, мистер Смит! А его нет...";
			link.l1 = "Вот чёрт, а где он!";
			link.l1.go = "SmithyTavern2_2";
		                   break;

                                                         case "SmithyTavern2_2":
			dialog.text = "Понятия не имею, вчера ушёл ничего не сказал. Прошу прощения у меня много дел.";
			link.l1 = "...";
		...
		// Addon 2016-1 Jason пиратская линейка
		case "mtraxx":
            dialog.text = "Хех, если ты намекаешь на нашего купца, что магазин содержит - то совершенно напрасно, я точно знаю, что он корабельным шёлком не торгует. Контрабандистам у  нас тут нечего делать, чай, таможни не держим. Ну а больше торговлей в Саванне-ла-Мар никто не промышляет - тут народ несколько другой собрался, ха-ха! Так что брешут люди.";
		...
		case "mtraxx_6":
			...
			pchar.quest.mtr_negril.win_condition.l1.date.hour  = 12;

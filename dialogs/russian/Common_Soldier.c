			...
			else
			{
				...
				if (GetNationRelation(sti(NPChar.nation), GetBaseHeroNation()) == RELATION_ENEMY && sti(NPChar.nation) != PIRATE)
				{
					...
					if (CheckNationLicence(HOLLAND))
					{
						...
						if (CheckCharacterPerk(pchar, "Trustworthy"))
						{   
					        link.l3 = RandPhraseSimple ("Солдат, взгляни на меня. Разве я похож на преступника?", "Совсем солдатня распоясалась... Придется поговорить об этом с комендантом!");
...
//============================== Взятка =========================
	...
	case "GiveMeSomeMoney":	
			...
			dialog.text = "Кто знает, капитан, может и договоримся...";
...
case "Bribe": 
		...
		break; 
case "Bribe_1": 
dialog.text = "Ступайте с миром, чтобы я больше вас здесь не видел!"; 
			Link.l1 = "Разумеется."; 

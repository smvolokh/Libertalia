...
void LAi_CharacterAttack()
{
	...
	if(isBlocked == true)  // to_do
			...
			//if(attack.id == pchar.id) Log_SetStringToLog("Шанс пробития блока: " + iChance + " процентов");
			if(rand(100) < iChance)
			{
			isBlocked = false;
			if(attack.id == pchar.id) Log_SetStringToLog("Вы пробили блок!");
			if(enemy.id == pchar.id) Log_SetStringToLog("Ваш блок пробит!");
			}
		}
	}
	...
	/* if(Silence)
	{
		...
		Log_TestInfo("Скрытность персонажа: " + GetSummonSkillFromName(attack, SKILL_SNEAK));
		
		if(IsAnotherCharactersAroundRadius(attack, radius) || !LAi_IsDead(enemy))
		{
			Log_TestInfo("Радиус: " + makeint(radius));
...
void LAi_CharacterFire()
{
	...
	// CSM --> выстрел произведен, разрешаем быструю смену мушкета через 0.75с
	if (attack.id == pchar.id)
...
// Осечка Пират Либерталии 03.04.21. идея Dolphin, но практически полностью переделана.
...
//if(attack.id == pchar.id) Log_SetStringToLog("Вероятность осечки: " + iChance + " процентов");
if(rand(100) < iChance)
   {
   if(attack.id == pchar.id) Log_SetStringToLog("Осечка!");
   return;
   }
// <-
	...
	if(CheckAttribute(attack, "chr_ai.misfire") && sti(attack.chr_ai.misfire) > 0 && rand(100) < sti(attack.chr_ai.misfire) && !HasSubStr(weapon.id, "mushket"))
	{
		...
		Log_SetStringToLog("Пистолету разорвало ствол! " + GetCharacterFullName(attack.id) + " выбросил" + GetSexPhrase("", "а") + " обломки.");
		return;
	}
...
bool LAi_CharacterIsFire()
{
	...
	if (chr.id == pchar.id)
	{
		if (stf(pchar.chr_ai.charge) >= 1.0)
			pchar.CSM.IsFireNow = true; // Огнестрел заряжен, запрещаем быструю смену мушкета
		else
			csmDA(pchar, "CSM.IsFireNow"); // Огнестрел не заряжен, можно юзать быструю смену мушкета
	}
	// <-- CSM

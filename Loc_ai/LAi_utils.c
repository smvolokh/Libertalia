...
void LAi_SetDefaultMiningAnimation(aref chr)	 // Для горного дела
{
	if(IsEntity(chr))
	{
	BeginChangeCharacterActions(chr);
				...
				if(CheckAttribute(chr, "equip.gun"))
				{
					if(rand(MOD_SKILL_ENEMY_RATE + 10) == 2)
					{
						...
						if(CheckAttribute(rItem,"quality") && rItem.quality != "excellent" && rItem.quality != B_UNIQUE) // ugeen --> на обычных трупах топовое оружие не даем !!!
						{
							...
							{ // залупу на воротник, а не кучи мушкетов
...
void MakePoisonAttack(aref attack, aref enemy)
{
	...
	AddCharacterHealth(enemy, -3.0) // Отравление портит здоровье на 3 ед
}
...
void LAi_ClawBeliar(ref chr, ref enemy, int damage) // "коготь Белиара"
{
	float x, y, z;
	int RandomSound = rand(2) // Рандомный звук	
	GetCharacterPos(chr, &x, &y, &z);
	CreateParticleSystemX("ShipExplode", x+rand(1), y+rand(1), z+rand(1), x, y, z, 0); // Создаем визуальный эффект
	...
	float dist = -1.0;
	if(findCh.chr_ai.group == LAI_GROUP_PLAYER || findCh.chr_ai.immortal == 1)  // Запретим атаковать бессмертных и НПЦ из группы игрока
		{
			damage = 0.0
		} 

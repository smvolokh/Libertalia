...
void LAi_UseAtidoteBottle(aref chr)
{
	...
	DelPerkFromActiveList("PoisonPerkA"); // Убираем перк, если действие яда окончено
	pchar.questTemp.poisonperk = "false"; // Анти-баг (чтоб перк работал как надо)
	...
	if(sGun != "") 
	{
		...
		for (int i = 0; i < iNum; i++)
		{
			...
			if(sBulletType == sBullet)
			{
				...
				rChar.chr_ai.Osechka	= rItm.type.(sAttr).Osechka;				
				...
				rItm.Osechka				= rItm.type.(sAttr).Osechka;
...
void LAi_AllCharactersUpdate(float dltTime)
{
	for(int i = 0; i < LAi_numloginedcharacters; i++)
	{
		...
		if(idx >= 0)
		{
			...
			if(CheckAttribute(pchar, "poison") && pchar.questTemp.poisonperk == "true")
			{
				AddPerkToActiveList("PoisonPerkA"); // Включаем всегда, когда ГГ отравлен
			}
			...
			if(chargemax > 0.0)
			{
				if(sti(chr_ai.chargeprc))
				{
					...
					if (bRechargePistolOnLine || !LAi_IsFightMode(chr) || chr.model.animation == "mushketer" || CheckCharacterPerk(pchar, "HT4"))
					{
